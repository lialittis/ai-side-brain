...
usage() {
  cat <<'EOF'
Usage: scripts/restore_personal_literature_radar_backup.sh [--dry-run] --target-root PATH ARCHIVE.tar.gz

Restores only Personal Literature Radar paths from a backup archive:
  indexes/
  memory/06_Logs/

The script refuses to restore into the live repository root unless
PERSONAL_RADAR_RESTORE_ALLOW_LIVE=1 is set.
EOF
}

DRY_RUN="${PERSONAL_RADAR_RESTORE_DRY_RUN:-0}"
TARGET_ROOT="${PERSONAL_RADAR_RESTORE_TARGET_ROOT:-}"
ARCHIVE_PATH=""
...
if [[ "$ABS_TARGET" == "$ROOT_DIR" && "${PERSONAL_RADAR_RESTORE_ALLOW_LIVE:-0}" != "1" ]]; then
  echo "Refusing to restore into the live repository root without PERSONAL_RADAR_RESTORE_ALLOW_LIVE=1." >&2
  exit 4
fi
...
allowed_member() {
  ...
  case "$member" in
    ...
    memory/06_Logs|memory/06_Logs/*) return 0 ;;
    *) return 1 ;;
  esac
}
...
if [[ "${#RESTORE_MEMBERS[@]}" -eq 0 ]]; then
  echo "No Personal Literature Radar paths found in archive." >&2
  exit 6
fi

if [[ "$DRY_RUN" == "1" ]]; then
  echo "Personal Literature Radar restore dry run"
...
echo "Personal Literature Radar restored into: $TARGET_ROOT"
