...
- Team Interests: weighted keyword sliders that define initial relevance scoring for new and newly analyzed papers.
- AI analysis: direct PDF links are downloaded only when they point to `.pdf` files without redirects; uploaded/downloaded PDFs are deduplicated by hash and malformed PDFs are rejected before OpenRouter. Manual links do not trigger PDF download; AI analyzes only the provided title, URL, and brief. AI receives the team tag catalog, reuses catalog tags first, and adds capped new tags only when needed. Without a key, the item is still saved and marked `AI: pending`; non-paper sources are archived as `AI: rejected_non_paper`.
- Library management: team members can edit displayed tags, relevance, and importance in place, add named comments on paper cards, sort by date/name/relevance/importance, and soft-remove items that remain recoverable at the end of the list for 24 hours.
